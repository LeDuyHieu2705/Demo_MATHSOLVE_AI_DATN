...
import com.math.solver.mathsolverapp.ui.cropper.CropImageView.CropShape
import com.math.solver.mathsolverapp.ui.cropper.CropImageView.Guidelines
...
internal class CropOverlayView @JvmOverloads constructor(
    ...
    private var mSetWindowAnimation: SetWindowAnimation? = null
    ...
    fun setWindowAnimation(listener: SetWindowAnimation?) {
        mSetWindowAnimation = listener
    }
    ...
    private fun initCropWindow() {
        ...
        Log.d(
            "sobutag",
            "initCropWindow: leftLimit: $leftLimit, topLimit: $topLimit, rightLimit: $rightLimit, bottomLimit: $bottomLimit"
        )
        Log.d("sobutag", "mInitialCropWindowPaddingRatio: $mInitialCropWindowPaddingRatio")
        ...
        val verticalPaddingTop = mInitialCropWindowPaddingRatio * (bottomLimit - topLimit)
        val verticalPaddingBottom =
            mInitialCropWindowPaddingRatio * (bottomLimit - topLimit) * 2.9.toFloat()
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            Log.d("sobutag", "initCropWindow 1")
            ...
            Log.d("sobutag", "initCropWindow 2")
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                rect.top = topLimit + verticalPaddingTop
                rect.bottom = bottomLimit - verticalPaddingBottom
            ...
            Log.d("sobutag", "initCropWindow 3")
        ...
        when (cropShape) {
            ...
            ->
                if (!isNonStraightAngleRotated) {
                    ...
                    mSetWindowAnimation?.onSetScanLoading(mCropWindowHandler.getRect(), left, right)
...
//                    mCropWindowChangeListener?.onSetScanLoading(mCropWindowHandler.getRect())
                }
    ...
    private fun drawRoundedCornerShape(
        canvas: Canvas,
        rect: RectF,
        cornerRadius: Float,
        cornerOffset: Float
        ...
        paint.strokeWidth = cornerOffset
        paint.style = Paint.Style.STROKE // Set stroke style
        ...
        canvas.drawLine(
            rect.left + cornerRadius,
            rect.top,
            rect.left + cornerRadius + cornerOffset,
            rect.top,
            paint
        )
        canvas.drawLine(
            ...
            rect.top + cornerRadius,
            rect.left,
            rect.top + cornerRadius + cornerOffset,
            paint
        )
        ...
        canvas.drawLine(
            rect.right - cornerRadius,
            rect.top,
            rect.right - cornerRadius - cornerOffset,
            rect.top,
            paint
        )
        canvas.drawLine(
        ...
        canvas.drawLine(
            ...
            rect.bottom - cornerRadius,
            rect.left,
            rect.bottom - cornerRadius - cornerOffset,
            paint
        )
    ...
    private fun drawRoundedCornerShape(
        ...
        canvas.drawLine(
            rect.left + cornerRadius,
            rect.top,
            rect.left + cornerRadius + cornerLength,
            rect.top,
            paint
        )
        canvas.drawLine(
            ...
            rect.top + cornerRadius,
            rect.left,
            rect.top + cornerRadius + cornerLength,
            paint
        )
        ...
        canvas.drawLine(
            rect.right - cornerRadius,
            rect.top,
            rect.right - cornerRadius - cornerLength,
            rect.top,
            paint
        )
        canvas.drawLine(
        ...
        canvas.drawLine(
            ...
            rect.bottom - cornerRadius,
            rect.left,
            rect.bottom - cornerRadius - cornerLength,
            paint
        )
    ...
    private fun drawRoundedCornerDrawable(
        canvas: Canvas,
        rect: RectF,
        context: Context
    ) {
        val icTopLeftDrawable = ContextCompat.getDrawable(context, R.drawable.ic_crop_top_left_1)
        val icTopRightDrawable = ContextCompat.getDrawable(context, R.drawable.ic_crop_top_right_1)
        val icBottomLeftDrawable = ContextCompat.getDrawable(context, R.drawable.ic_crop_bottom_left_1)
        val icBottomRightDrawable = ContextCompat.getDrawable(context, R.drawable.ic_crop_bottom_right_1)

        val marginSpace = 14

        // Top left corner
        val icTopLeftWidth = icTopLeftDrawable?.intrinsicWidth ?: 0
        val icTopLeftHeight = icTopLeftDrawable?.intrinsicHeight ?: 0
        icTopLeftDrawable?.setBounds(
            (rect.left - marginSpace).toInt(),
            (rect.top - marginSpace).toInt(),
            (rect.left + icTopLeftWidth - marginSpace).toInt(),
            (rect.top + icTopLeftHeight - marginSpace).toInt()
        )
        icTopLeftDrawable?.draw(canvas)

        // Top right corner
        val icTopRightWidth = icTopRightDrawable?.intrinsicWidth ?: 0
        val icTopRightHeight = icTopRightDrawable?.intrinsicHeight ?: 0
        icTopRightDrawable?.setBounds(
            (rect.right - icTopRightWidth + marginSpace).toInt(),
            (rect.top - marginSpace).toInt(),
            (rect.right + marginSpace).toInt(),
            (rect.top + icTopRightHeight - marginSpace).toInt()
        )
        icTopRightDrawable?.draw(canvas)

        // Bottom left corner
        val icBottomLeftWidth = icBottomLeftDrawable?.intrinsicWidth ?: 0
        val icBottomLeftHeight = icBottomLeftDrawable?.intrinsicHeight ?: 0
        icBottomLeftDrawable?.setBounds(
            (rect.left - marginSpace).toInt(),
            (rect.bottom - icBottomLeftHeight + marginSpace).toInt(),
            (rect.left + icBottomLeftWidth - marginSpace).toInt(),
            (rect.bottom + marginSpace).toInt()
        )
        icBottomLeftDrawable?.draw(canvas)

        // Bottom right corner
        val icBottomRightWidth = icBottomRightDrawable?.intrinsicWidth ?: 0
        val icBottomRightHeight = icBottomRightDrawable?.intrinsicHeight ?: 0
        icBottomRightDrawable?.setBounds(
            (rect.right - icBottomRightWidth + marginSpace).toInt(),
            (rect.bottom - icBottomRightHeight + marginSpace).toInt(),
            (rect.right + marginSpace).toInt(),
            (rect.bottom + marginSpace).toInt()
        )
        icBottomRightDrawable?.draw(canvas)
...
//
//
//        val icCenterWidth = icCenterDrawable?.intrinsicWidth ?: 0
//        val icCenterHeight = icCenterDrawable?.intrinsicHeight ?: 0
//        icCenterDrawable?.setBounds(
//            (centerX - icCenterWidth / 2).toInt(),
//            (centerY - icCenterHeight / 2).toInt(),
//            (centerX + icCenterWidth / 2).toInt(),
//            (centerY + icCenterHeight / 2).toInt()
//        )
//        icCenterDrawable?.draw(canvas)
    }
    ...
    private fun drawCornerShape(
        ...
        when (cornerShape) {
            ...
            CropImageView.CropCornerShape.RECTANGLE -> drawRoundedCornerDrawable(
                canvas,
                rect,
                context
            )
...
//        Log.d("sobutag","onActionUp")
    ...
    internal fun interface SetWindowAnimation {
        fun onSetScanLoading(rect: RectF, left: Float, right: Float)
    }
